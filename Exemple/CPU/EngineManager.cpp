...
void EngineManager::OnUpdateWindowEngine() {
    ...
    std::default_random_engine generator;
    std::uniform_real_distribution<float> positionDist(-0.1f, 0.1f);
    std::uniform_real_distribution<float> velocityDist(1.0f, 3.0f);
    ...
    for (auto& p : particles) {
        ...
        if (p.life <= 0.0f) {
            // Régénération si la particule est "morte"
            p.pos = glm::vec3(positionDist(generator), 0.0f, positionDist(generator));
            float upwardVelocity = velocityDist(generator);
            p.dir = glm::vec3(positionDist(generator), upwardVelocity, positionDist(generator));
            p.life = lifeDist(generator);  // Nouvelle durée de vie
            p.maxLife = p.life;
        } else {
            // Mise à jour normale de la direction avec gravité
    ...
    // Transmission des positions mises à jour au GPU
    glBindBuffer(GL_ARRAY_BUFFER, VBO3);
    glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(glm::vec3) * nbParticules, &particles[0].pos);
    glBindBuffer(GL_ARRAY_BUFFER, 0);
